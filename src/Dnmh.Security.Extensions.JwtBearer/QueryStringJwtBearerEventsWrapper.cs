...
namespace Dnmh.Security.Extensions.JwtBearer
{
    ...
    public class QueryStringJwtBearerEventsWrapper : JwtBearerEventsWrapperBase
    {
        ...
        public override Task MessageReceived(MessageReceivedContext context)
        {
            ...
            if (!context.Request.Query.TryGetValue(QueryStringParameterName, out values))
            {
            ...
            if (values.Count > 1)
            {
                ...
                context.Fail(
                    $"Only one '{QueryStringParameterName}' query string parameter can " +
            ...
            if (string.IsNullOrWhiteSpace(token))
            {
                context.Response.StatusCode = (int)HttpStatusCode.Unauthorized;
                context.Fail(
                    $"The '{QueryStringParameterName}' query string parameter was " +
